...
<section class="subheader">
  <div class="container">
    <h1>Properties For {{ ucfirst($detail->listed_for) }}</h1>
    <div class="breadcrumb right">Home <i class="fa fa-angle-right"></i> <a class="current">Properties For {{ ucfirst($detail->listed_for) }}</a></div>
    <div class="clear"></div>
  </div>
</section>
			...
			<div class="property-single-item property-main">
				<div class="property-header">
					<div class="property-title">
						<h4>{{ $detail->title }}</h4>
            <a href="callto:+91{{ $detail->phone }}" class="btn default_btn"><i class="fa fa-phone" aria-hidden="true"></i>Call Us To Get Price</a>
						<p class="property-address"><i class="fa fa-map-marker icon"></i>{{ $detail->address }}</p>
            <div class="clear"></div>
					</div>
					<div class="property-single-tags">
					
						<div class="property-type right">Property Type: <a href="#">{{ $detail->cattitle }}</a></div>
					</div>
				</div>
        ...
        <div class="property-gallery">
          ...
          <div class="slider slider-property-gallery">
          	...
          	@if($property_image->property_image)
            	<div class="slide"><img style="height:492px !important;" src="{{ $property_image->property_image }}" alt="Properties for sale" /></div>
            @endif
            @endforeach
          </div>
          ...
          <div class="slider property-gallery-pager">
          	@foreach($property_images as $property_image)
          	@if($property_image->property_image)
            	<a class="property-gallery-thumb"><img src="{{ $property_image->property_image }}" alt="Rental properties" /></a>
            @endif
            @endforeach
          </div>
			...
			<div class="widget property-single-item property-description content">
		        ...
		        <div id="tabs-2" class="ui-tabs-hide">
		        	@if($detail->floor_plan)
		          	<a href="#"><img src="{{ $detail->floor_plan }}" alt="Property consultant" /></a>
		          @endif
		        </div>
		        <div id="tabs-3" class="ui-tabs-hide">
		        	...
		        	@if($attachment->attachment)
		          	<a href="{{ $attachment->attachment }}" download="{{ $attachment->attachment }}"><i class="fa fa-file-o icon"></i> File {{ $file_count }}</a><br/><br/>
		          @endif
            ...
            <p>The beautiful Aura Solis's commencement certificate has not been granted. In addition to this, the occupancy certificate not granted. Aura Solis has been developed by the well-known builder Eminent Spaces. Aura Solis is creating a benchmark with top-in-class amenities including Power Back Up, Multipurpose Courts, Kids Play Area, Indoor Games Room, Jogging and Strolling Track, Waste Disposal, Maintenance Staff, Visitor Parking, Service/Goods Lift, Water Storage. Aura Solis At Wanawari Near Naren Hills Opp Mohammadwadi Road, Near Rims School Wanawari Annex Pune is the official address of this project. 411040 is the area pincode under which this project falls. Aura Solis is where fine living meets seamless connectivity to provide you comfort living.</p>
			        ...
			        <div id="tabs-2" class="ui-tabs-hide">
			          <a href="#"><img src="{{ env('ASSET_URL1') }}/public/frontend/images/floor-plan1.jpg" alt="Property consultant" /></a>
			        </div>
			        <div id="tabs-3" class="ui-tabs-hide">
			...
			<div class="widget property-single-item property-amenities">
				...
				<ul class="amenities-list">
					...
					@if(in_array($amen->id, $detail->amenities))
						<li><i class="fa fa-check icon"></i> {{ ucfirst($amen->name) }}</li>
					@else
						<li><i class="fa fa-close icon"></i> {{ ucfirst($amen->name) }}</li>
					@endif
					@endforeach
				</ul>
			</div>
			...
			<div class="widget property-single-item property-amenities">
				...
				<ul class="amenities-list">
					...
					@if(in_array($feat->id, $detail->features))
						<li><i class="fa fa-check icon"></i> {{ ucfirst($feat->name) }}</li>
					@else
						<li><i class="fa fa-close icon"></i> {{ ucfirst($feat->name) }}</li>
					@endif
					@endforeach
				</ul>
			</div>
			  ...
			  <div class="widget-content box mt30px" >
				<form id="inquiryForm">
				  ...
				  <div class="form-block">
					...
					<input type="hidden" name="product_id" value="{{ $detail->id }}" required />
